...
from app.models.models_master import LoginRights as model
from app.schemas.admin import schema_loginrights as schema
...
def create(request:schema.add,db: Session,current_user):
    try:
        ...
        logging.error(f"loginrights in create: {str(e)}")
...
def get_id(PlaceCode:int,db:Session):
    try:
        ...
        logging.error(f"loginrights in get_id: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Server Error")
...
def get_all(db:Session):
    try:
        ...
        logging.error(f"loginrights in get_all: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Server Error")
