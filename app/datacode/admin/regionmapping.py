...
from app.models.models_master import EmplRegionMapping as model
...
def create(request:schema.add,db: Session,current_user):
    try:
        Check=db.query(model).filter(model.EmployeeCode == request.EmployeeCode)
        if Check.first():
                db.query(model).filter(model.EmployeeCode == request.EmployeeCode).delete()
        ...
        logging.error(f"emplregionmapping in create: {str(e)}")
...
def get_id(PlaceCode:int,db:Session):
    try:
        ...
        logging.error(f"emplregionmapping in get_id: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Server Error")
...
def get_all(db:Session):
    try:
        ...
        logging.error(f"emplregionmapping in get_all: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Server Error")
