...
from app.schemas.admin import schema_LoginLocChnlMapping as schema
...
def create(request:schema.add,db: Session,current_user):
    try:
        ...
        logging.error(f"LoginLocChnlMapping in create: {str(e)}")
...
def get_id(PlaceCode:int,db:Session):
    try:
        ...
        if not data:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
                                detail=f"mapping  not available")
        return data
    except HTTPException as http_exception:
        raise http_exception
    except Exception as e:
        logging.error(f"LoginLocChnlMapping in get_id: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Server Error") 
...
def get_all(db:Session):
    try:
        ...
        logging.error(f"LoginLocChnlMapping in get_all: {str(e)}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Server Error") 
