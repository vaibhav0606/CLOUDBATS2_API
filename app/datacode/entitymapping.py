...
from app.models.models_master import EntityMapping as model
from app.schemas import schema_entitymapping as schema
from fastapi import HTTPException,status
from datetime import datetime

def create(request:schema.add,db: Session,current_user):
    Check=db.query(model).filter(model.EntityCode == request.EntityCode)
    if Check.first():
            db.query(model).filter(model.EntityCode == request.EntityCode).delete()
...
def get_id(EntityCode:int,db:Session):
    user = db.query(model).filter(model.EntityCode == EntityCode).first()
    if not user:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND,
