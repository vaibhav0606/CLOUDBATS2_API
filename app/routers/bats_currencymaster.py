...
def get_id(CurrencyCode:int,db: Session = Depends(database.Connect_db)):
    return currencymaster.get_id(CurrencyCode,db)

@router.post('/', response_model=schema_currencymaster.addcurrency)
def create_currency(request: schema_currencymaster.addcurrency,db: Session = Depends(database.Connect_db)):
    return currencymaster.create(request,db)

@router.put('/{CurrencyCode}', status_code=status.HTTP_202_ACCEPTED)
def update(CurrencyCode:int, request: schema_currencymaster.updatecurrency, db: Session = Depends(database.Connect_db)):
