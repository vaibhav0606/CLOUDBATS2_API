...
from app.datacode.admin import placemaster as datacode
...
@router.get('/drop/',response_model=List[schema.showplace])
def get_drop(db: Session = Depends(database.Connect_db),current_user: userschema.User = Depends(oauth2.get_current_user)):#current_user: userschema.User = Depends(oauth2.get_current_user)
